...
module.exports = class IndexDbController extends EventEmitter {
  ...
  open () {
    return new Promise((resolve, reject) => {
      const dbOpenRequest = this.dbObject.open(this.key, this.version)
  ...
  requestObjectStore (key, type = 'readonly') {
    return new Promise((resolve, reject) => {
      const dbReadWrite = this.db.transaction(key, 'readonly')
  ...
  migrate () {
    ...
    // this.objectStore = this.db.createObjectStore('testground')
  }
